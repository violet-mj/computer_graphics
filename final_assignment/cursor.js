...
var numVertices  = 216;
var objectZCoord = 0.0;
var pointsArray = [];
var normalsArray = [];
var mouseCoords = [0, 0, 0];

function offsetToClip(coords) {
   return coords.map((coord, index) => {
     switch(index) {
       case 0:
         return coord / 512 * 2 -1
       case 1: 
         return - (coord / 512 * 2) + 1
       case 2: 
         return 4
...
vertices = vertices.map((data)=> {
   return vec4(data[0] / 3, data[1] / 3, data[2] / 3, 1.0)
})

const vertices_rr = vertices.map((data) => {
   return vec4(data[0] + 0.3, data[1] + 0.3, data[2], 1.0) 
})

vertices = [...vertices, ...vertices_rr]
...
window.onload = function init() {
    ...
    canvas.addEventListener("mousemove", (e) => {
       mouseCoords = offsetToClip([e.offsetX, e.offsetY, 0])
   ...
   const drawDogHead = (x, y, modelView) => {
      // eye = vec3(radius*Math.sin(p_theta)*Math.cos(phi),
      
      const xDiff = x - mouseCoords[xAxis]
      const yDiff = y - mouseCoords[yAxis]
      const zDiff = objectZCoord - mouseCoords[zAxis]
      // console.log(xDiff, yDiff, zDiff)
      
      theta[xAxis] = Math.floor(toDegrees(Math.atan(-yDiff / zDiff)))
      const yAngle =  Math.floor(toDegrees(Math.atan( zDiff / xDiff))) / 10
      theta[yAxis] = mouseCoords[0] > 0 ? yAngle : yAngle +18
      // theta[yAxis] = 6
      const zAngle = Math.floor(toDegrees(Math.atan( yDiff / xDiff)))
      theta[zAxis] = mouseCoords[0] > 0 ? zAngle : zAngle - 180
      // console.log(theta)
      // theta = [352,0,10]
      eye = vec3(0,0,0)
